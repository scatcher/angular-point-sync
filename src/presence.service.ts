...
export class PresenceService {
      ...
      (initializationParamsObject: ISyncServiceInitializationParams) => {
        ...
        connectedRef.on("value", (snap) => {
          if (snap.val() === true) {
            ...
            const activeSessionObject = service.$firebaseObject(
              service.sessionConnection
            );
            deferred.resolve(activeSessionObject);

            // watch for events
            service.watchForReloadEvent(activeSessionObject);
            service.watchForNotifications(
              this.currentUserId,
              activeSessionObject.$id
            );
          }
        });
      }
  ...
  displayUserNotification(notification: IUserNotification): void {
    if (toastr) {
      toastr[notification.toastType](
        notification.message,
        notification.title,
        notification.toastrOptions
      );
    } else {
  ...
  getLogsForUser(userId: number, quantity = 100) {
    return serviceIsInitialized.then(
      (initializationParamsObject: ISyncServiceInitializationParams) => {
        ...
        return service
          .$firebaseArray(userLogsRef)
          .$loaded() as SyncUserSessionLog[];
      }
    );
  }

  getSessionNotificationsArray(
    userId: number,
    sessionKey: string
    ...
    return service
      .getSessionConnectioUrl(userId, sessionKey)
      .then((sessionConnectionUrl) => {
        const notificationsRef = this.sessionConnection.root.child(
          sessionConnectionUrl + "/notifications"
        );
        return service.$firebaseArray(notificationsRef).$loaded();
      });
  }

  getSessionConnectioUrl(
    userId: number,
    sessionKey: string
  ): ng.IPromise<string> {
    return serviceIsInitialized.then(
      (initializationParamsObject: ISyncServiceInitializationParams) => {
        return (
          initializationParamsObject.firebaseRef.path +
          "/monitoring/users/" +
          userId +
          "/connections/" +
          sessionKey
  ...
  sendUserNotification(
    ...
    this.getSessionNotificationsArray(userId, sessionKey).then(
      ...
      (notificationArray) =>
        notificationArray.$watch((eventObject: any) => {
          ...
          if (eventObject.event === "child_added") {
            _.each(
              notificationArray,
              (notification: IUserNotification, index: any) => {
